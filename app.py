...
def calculate_total_price(
    ...
    # Define a dictionary to hold the prices for pints and quarts of mole sauces and salsa selection
    pints_quarts_prices = {
        "Pints": {
            "Salsa": 5.50,
            "Amarillo": 10.99,
...
def download_success_docx():
    ...
    order_data = session.get('order_data', {})
      # Debug: Print the contents of the order_data dictionary
    print("Order Data in Session:", order_data)
    ...
    if carnitas_found:
        ...
        for run in carnitas_line.runs:
            run.bold = True
                                ...
                                # Add Mole details
    for mole_flavor, quantity in mole_data.items():
        print(f"Processing mole flavor: {mole_flavor}")  # Debug line
        print(f"Quantity: {quantity}")  # Debug line
        mole_info = mole_flavor.split(' (', 1)
        print(f"Mole Info: {mole_info}")  # Debug line
        mole_name = mole_info[0]
        protein_choice = mole_info[1][:-1] if len(mole_info) > 1 else ''
        print(f"Mole Name: {mole_name}, Protein Choice: {protein_choice}")  # Debug line
        price = quantity * mole_prices.get(mole_name.strip(), 0)  # Debug line
        print(f"Price for {mole_name}: {price}")  # Debug line
